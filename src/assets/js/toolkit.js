				...
				if (typeof this._config.offset === 'function') {
					offsetConf.fn = function(data) {
						data.offsets = _extends(
							{},
							data.offsets,
							_this10._config.offset(data.offsets) || {}
						);
						return data;
					};
				} else {
				...
				$(document)
					...
					.on(Event.FOCUSIN, function(event) {
						if (
							...
							_this14._element !== event.target &&
							$(_this14._element).has(event.target).length === 0
						) {
				...
				if (this._isBodyOverflowing) {
					...
					$(Selector.FIXED_CONTENT).each(function(index, element) {
						...
						$(element)
							...
							.css(
								'padding-right',
								parseFloat(calculatedPadding) + _this19._scrollbarWidth + 'px'
							);
					});
					...
					$(Selector.STICKY_CONTENT).each(function(index, element) {
						...
						$(element)
							...
							.css(
								'margin-right',
								parseFloat(calculatedMargin) - _this19._scrollbarWidth + 'px'
							);
					});
		...
		var Tooltip = (function() {
			...
			Tooltip.prototype._setListeners = function _setListeners() {
				...
				triggers.forEach(function(trigger) {
					if (trigger === 'click') {
						$(_this23.element).on(
							_this23.constructor.Event.CLICK,
							_this23.config.selector,
							function(event) {
						...
						var eventIn =
							trigger === Trigger.HOVER
								? _this23.constructor.Event.MOUSEENTER
								: _this23.constructor.Event.FOCUSIN;
						var eventOut =
							trigger === Trigger.HOVER
								? _this23.constructor.Event.MOUSELEAVE
								: _this23.constructor.Event.FOCUSOUT;

						$(_this23.element)
							.on(eventIn, _this23.config.selector, function(event) {
 ...
 * ======================================================================== */
				...
				$(this._element)
					.css({
						'-webkit-transition':
							...
							this._config.easing,
						'-ms-transition':
							'-ms-transform ' + this._config.duration + 'ms ' + this._config.easing,
						transition:
							'transform ' + this._config.duration + 'ms ' + this._config.easing
					})
					.css({
	...
	var ImageGrid = (function($) {
		...
		var ImageGrid = (function() {
			...
			ImageGrid.prototype._applyModifications = function _applyModifications(
				$obj,
				isNotLast
			) {
				var css = {
	...
	var Zoom = (function($) {
		...
		var ZoomService = (function() {
			...
			ZoomService.prototype._activeZoomClose = function _activeZoomClose(
				forceDispose
			) {
